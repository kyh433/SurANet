...
class EdgeDecoder2(nn.Module):
    ...
    def forward(self, x1, x2, x3,x4):
        ...
        # x4_2_rfb = self.rfb4(x4_2) #18,32,88,88
        ...
        e4_2=x
        return x2_2_conv,x3_2_conv,x4_2_conv,e2_2,e3_2,e4_2,x
        # return x2_2_rfb,x3_2_rfb,x4_2_rfb,e2_2,e3_2,e4_2,x
class SurroundDecoder(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_concat4 = BasicConv2d(4*channel, 4*channel, 3, padding=1)
        self.conv4 = BasicConv2d(4*channel, 4*channel, 3, padding=1)
        self.conv5 = nn.Conv2d(4*channel, out_channels=3, kernel_size=1)

        self.convg2_1 = BasicConv2d(2*channel, 2*channel, 3, padding=1)
        self.convg2_2 = nn.Conv2d(2*channel, out_channels=3, kernel_size=1)
        self.convg3_1 = BasicConv2d(3*channel, 3*channel, 3, padding=1)
        self.convg3_2 = nn.Conv2d(3*channel, out_channels=3, kernel_size=1)
    ...
    def forward(self, x1, x2, x3,x4):
        ...
        g4_2=x
        return x2_2_conv,x3_2_conv,x4_2_conv,g2_2,g3_2,g4_2,x
        # return x2_2_rfb,x3_2_rfb,x4_2_rfb,g2_2,g3_2,g4_2,x
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        F3_edge,F2_edge,F1_edge,_,_,_,S_edge = self.EDD2(x4_rfb, x3_rfb, x2_rfb,x1_rfb)
        S_edge_pred = F.interpolate(S_edge, scale_factor=4, mode='bilinear')    # Sup-1 (bs, 1, 88, 88) -> (bs, 1, 352, 352)

        _,_,_,g3_sur3,g2_sur3,g1_sur3,S_sur3 = self.SRD(x4_rfb, x3_rfb, x2_rfb,x1_rfb)
        g3_sur,g2_sur,g1_sur,S_sur=g3_sur3[:,2,None,:,:],g2_sur3[:,2,None,:,:],g1_sur3[:,2,None,:,:],S_sur3[:,2,None,:,:]
        S_sur_pred3 = F.interpolate(S_sur3, scale_factor=4, mode='bilinear')    # Sup-1 (bs, 3, 88, 88) -> (bs, 3, 352, 352)
        S_sur_pred=S_sur_pred3[:,2,None,:,:]
        # ---- reverse stage 5 ----
        guidance_g = F.interpolate(S_g, scale_factor=0.125, mode='bilinear')
        guidance_e = F.interpolate(S_edge, scale_factor=0.125, mode='bilinear')
        guidance_s = F.interpolate(S_sur, scale_factor=0.125, mode='bilinear')
        # ra4_feat = self.RS5(x4_rfb, guidance_g)
        ra4_feat = self.NewRS5(x4_rfb, guidance_s)
        S_5 = ra4_feat + guidance_g + guidance_e - guidance_s
        S_5_pred = F.interpolate(S_5, scale_factor=32, mode='bilinear')  # Sup-2 (bs, 1, 11, 11) -> (bs, 1, 352, 352)

        # x3_rfb_ = torch.cat((x3_rfb,F3_edge,F3_sur), 1)  #32+32+32
        ...
        guidance_s5 = F.interpolate(guidance_s, scale_factor=2, mode='bilinear')
        # ra3_feat = self.RS4(x3_rfb_, guidance_5)
        ra3_feat = self.NewRS4(x3_rfb_, guidance_s5)
        S_4 = ra3_feat + guidance_5 - g3_sur
        S_4_pred = F.interpolate(S_4, scale_factor=16, mode='bilinear')  # Sup-3 (bs, 1, 22, 22) -> (bs, 1, 352, 352)

        # x2_rfb_ = torch.cat((x2_rfb,F2_edge,F2_sur), 1)  #32+32+32
        ...
        # ra2_feat = self.RS3(x2_rfb_, guidance_4)
        ra2_feat = self.NewRS3(x2_rfb_, guidance_s4)
        S_3 = ra2_feat + guidance_4 - g2_sur
        S_3_pred = F.interpolate(S_3, scale_factor=8, mode='bilinear')   # Sup-4 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        # x1_rfb_ = torch.cat((x1_rfb,F1_edge,F1_sur), 1)  #32+32+32
        ...
        # ra1_feat = self.RS2(x1_rfb_, guidance_3)
        ra1_feat = self.NewRS2(x1_rfb_, guidance_s3)
        S_2 = ra1_feat + guidance_3 - g1_sur
        S_2_pred = F.interpolate(S_2, scale_factor=4, mode='bilinear')   # Sup-4 (bs, 1, 88, 88) -> (bs, 1, 352, 352)

        # S_edge_pred=self.EDD(x4, x3, x2, x1)
        S_2_pred=S_2_pred+S_edge_pred - S_sur_pred

        # diyplot3(xx,S_g_pred,S_edge_pred,ra4_feat,S_5_pred,ra3_feat,S_4_pred,ra2_feat,S_3_pred,ra1_feat,S_2_pred,S_sur_pred)

        return S_g_pred, S_5_pred, S_4_pred, S_3_pred,S_2_pred,S_edge_pred,S_sur_pred3
