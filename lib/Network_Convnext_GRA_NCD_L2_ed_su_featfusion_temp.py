...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        self.conv_concat3 = BasicConv2d(3*channel, channel, 3, padding=1)
        self.conv_concat2 = BasicConv2d(3*channel, channel, 3, padding=1)
        self.conv_concat1 = BasicConv2d(3*channel, channel, 3, padding=1)
    ...
    def forward(self, x):
        ...
        F3_edge,F2_edge,F1_edge,g3_edge,g2_edge,g1_edge,S_edge = self.EDD2(x4_rfb, x3_rfb, x2_rfb,x1_rfb)
        S_edge_pred = F.interpolate(S_edge, scale_factor=4, mode='bilinear')    # Sup-1 (bs, 1, 88, 88) -> (bs, 1, 352, 352)

        F3_sur,F2_sur,F1_sur,g3_sur,g2_sur,g1_sur,S_sur = self.SRD(x4_rfb, x3_rfb, x2_rfb,x1_rfb)
        ...
        S_5 = ra4_feat + guidance_g + guidance_e + guidance_s_
        ...
        g3_sur_ = -1 * (torch.sigmoid(g3_sur)) + 1
        S_4 = ra3_feat + guidance_5 + g3_sur_
        ...
        g2_sur_ = -1 * (torch.sigmoid(g2_sur)) + 1
        S_3 = ra2_feat + guidance_4 + g2_sur_
