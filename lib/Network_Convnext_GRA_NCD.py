...
class Network_(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        # self.convnext = convnext_small(pretrained=imagenet_pretrained)
    ...
    def forward(self, x):
        # Feature Extraction
        # x = self.convnext.conv1(x)
        # x = self.convnext.bn1(x)
        # x = self.convnext.relu(x)
        # x = self.convnext.maxpool(x)      # bs, 64, 88, 88
        # x1 = self.convnext.layer1(x)      # bs, 256, 88, 88
        # x2 = self.convnext.layer2(x1)     # bs, 512, 44, 44
        # x3 = self.convnext.layer3(x2)     # bs, 1024, 22, 22
        # x4 = self.convnext.layer4(x3)     # bs, 2048, 11, 11
