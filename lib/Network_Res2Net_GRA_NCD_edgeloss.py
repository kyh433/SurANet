...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        # S_5    #(bs, 1, 11, 11)
        # S_g    #(bs, 1, 44, 44)
        S_5_up = F.interpolate(S_5, scale_factor=2, mode='bilinear')
        S_4_delta=abs(S_4-S_5_up)
        S_4_up = F.interpolate(S_4, scale_factor=2, mode='bilinear')
        S_3_delta=abs(S_3-S_4_up)

        return S_g_pred, S_5_pred, S_4_pred, S_3_pred, S_4_delta, S_3_delta
