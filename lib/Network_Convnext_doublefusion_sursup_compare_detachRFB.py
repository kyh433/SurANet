...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        self.conv_concat3_s = BasicConv2d(2*channel, channel, 3, padding=1)
        self.conv_concat2_s = BasicConv2d(2*channel, channel, 3, padding=1)
        self.conv_concat1_s = BasicConv2d(2*channel, channel, 3, padding=1)
    def forward(self, x):
        # Feature Extraction
        img=x
        ...
        # x1_rfb_s = x1_rfb.detach()
        # x2_rfb_s = x2_rfb.detach()
        # x3_rfb_s = x3_rfb.detach()
        # x4_rfb_s = x4_rfb.detach()
        ...
        x3_rfb_g = self.conv_concat3(x3_rfb_)
        x3_rfb_s = self.conv_concat3_s(x3_rfb_)

        ra3_feat_g = self.RS4(x3_rfb_g, guidance_5)
        ra3_feat_s = self.NewRS4(x3_rfb_s, g3_sur)
        ...
        x2_rfb_g = self.conv_concat2(x2_rfb_)
        x2_rfb_s = self.conv_concat2_s(x2_rfb_)

        ra2_feat_g = self.RS3(x2_rfb_g, guidance_4)
        ra2_feat_s = self.NewRS3(x2_rfb_s, g2_sur)
        ...
        x1_rfb_g = self.conv_concat1(x1_rfb_)        
        x1_rfb_s = self.conv_concat1_s(x1_rfb_)        
        
        ra1_feat_g = self.RS2(x1_rfb_g, guidance_3)
        ra1_feat_s = self.NewRS2(x1_rfb_s, S_sur)
        ...
        return S_g_pred, S_5_pred, S_4_pred, S_3_pred,S_2_pred,S_edge_pred,S_sur_pred,g3_sur_pred,g2_sur_pred,x3_rfb_s,x2_rfb_s,x1_rfb_s,S_4,S_3,S_2
